...
const Header = ({ onStartMonitoring }: HeaderProps) => {
    ...
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-[rgba(0,212,255,0.15)] bg-[rgba(8,12,22,0.92)] backdrop-blur-md">
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-sm bg-[var(--neon)] text-[hsl(220,30%,6%)] rounded-md hover:bg-[rgba(0,212,255,0.85)] transition-all duration-200 font-bold shadow-[var(--neon-glow)]"
            >
              Начать мониторинг
            </button>
          </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden border-t border-[rgba(0,212,255,0.15)] bg-[rgba(8,12,22,0.98)] px-4 py-4 space-y-3">
          ...
          <button
            onClick={() => { setIsMenuOpen(false); onStartMonitoring(); }}
            className="w-full mt-3 px-4 py-2 text-sm bg-[var(--neon)] text-[hsl(220,30%,6%)] rounded-md font-bold"
          >
            Начать мониторинг
          </button>
        </div>
